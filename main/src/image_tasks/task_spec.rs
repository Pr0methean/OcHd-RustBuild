    ...
    fn add_to(
        ...
        let function: BasicTaskFunction<MaybeFromPool<Pixmap>> = match self {
            ...
            ToPixmapTaskSpec::FromSvg { source } => {
                ...
                Box::new(move || Ok(Arcow::Sharing(from_svg(source, tile_size)?.into())))
            }
            ToPixmapTaskSpec::StackLayerOnColor {
                ...
                Box::new(move || {
                    ...
                    fg_image.consume(|mut out_image| {
                        ...
                        Ok(Arcow::sharing_from(out_image))
                    })
                })
            }
            ...
            UpscaleFromGridSize { base } => {
                ...
                Box::new(move || {
                    ...
                    Ok(Arcow::sharing_from(upscale_image(base_image.deref(), tile_size)?))
                })
            }
        };
    ...
    fn add_to(
        ...
        let function: BasicTaskFunction<MaybeFromPool<Mask>> = match self {
            ...
            ToAlphaChannelTaskSpec::FromPixmap { base } => {
                ...
                Box::new(move || {
                    let base_image = base_future.into_result()?;
                    base_image.consume(|base_image| Ok(Arcow::sharing_from(pixmap_to_mask(&base_image))))
                })
            }
            StackAlphaOnAlpha {
                background,
                ...
                Box::new(move || {
                    ...
                    bg_mask.consume(|mut out_mask| {
                        ...
                        Ok(Arcow::sharing_from(out_mask))
                    })
                })
            }
            ToAlphaChannelTaskSpec::StackAlphaOnBackground {
                ...
                Box::new(move || {
                    ...
                    fg_arc.consume(|mut fg_image| {
                        ...
                        Ok(Arcow::sharing_from(fg_image))
                    })
                })
            }
            ToAlphaChannelTaskSpec::UpscaleFromGridSize { base } => {
                ...
                Box::new(move || {
                    ...
                    Ok(Arcow::sharing_from(upscale_mask(base_mask.deref(), tile_size)?))
                })
            }
        };
        info!("Adding node: {}", name);
            ...
            FileOutputTaskSpec::PngOutput { base, .. } => {
                ...
                Box::new(move || {
                    ...
                    base_result.consume(|image| {
                        Ok(Arcow::sharing_from(png_output(
                            image,
                            color_type,
                            bit_depth,
                            destination_path,
        ...
        match neighbor {
            ...
            SpecifiedColors(neighbor_colors) => match self {
                ...
                SpecifiedColors(self_colors) => {
                    ...
                    SpecifiedColors(Arcow::sharing_from(combined_colors))
                }
            },
        }
    }
        ...
        let alpha_vec: BasicTask<U8BitSet> = match self {
            ToAlphaChannelTaskSpec::MakeSemitransparent { alpha, base } => {
                ...
                CloneableLazyTask::new(
                    name,
                    Box::new(move || {
                        Ok(Arcow::sharing_from(multiply_alpha_vec(*base_alphas_task.into_result()?, alpha)))
                    }),
                )
            }
            ...
            ToAlphaChannelTaskSpec::StackAlphaOnBackground {
                ...
                CloneableLazyTask::new(
                    name,
                    Box::new(move || {
                        Ok(Arcow::sharing_from(stack_alpha_vecs(
            ...
            ToPixmapTaskSpec::PaintAlphaChannel { color, base } => {
                ...
                Left(Box::new(move || {
                    Ok(SpecifiedColors({
                        ...
                        Arcow::sharing_from(colored_alphas)
                    }))
                }))
            }
            ToPixmapTaskSpec::StackLayerOnColor {
                ...
                Left(Box::new(move || {
                    Ok(fg_task
                        .into_result()?
                        .stack_on(&SpecifiedColors(Arcow::sharing_from(vec![background])), pixels + 1))
                }))
            }
            ToPixmapTaskSpec::StackLayerOnLayer {
                background,
        ...
        let task = match desc {
            Left(function) => {
                ...
                actual color count", name_inner, pixels);
                                    ...
                                    actual_colors.sort();
                                    actual_colors.dedup();
                                    Arcow::sharing_from(SpecifiedColors(Arcow::sharing_from(actual_colors)))
                                } else {
                                    Arcow::sharing_from(SpecifiedColors(colors.clone()))
                                }
                            }
