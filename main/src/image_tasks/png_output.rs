...
use crate::{anyhoo, TOKIO_RUNTIME};
use crate::image_tasks::task_spec::{AsyncOutputResult};
...
#[instrument]
pub fn png_output(image: Pixmap, file: PathBuf) -> AsyncOutputResult {
    ...
    Ok(Arc::new(Box::new(TOKIO_RUNTIME.spawn(
        async move {
...
pub fn copy_out_to_out(source: PathBuf, dest: PathBuf) -> AsyncOutputResult {
...
pub fn copy_in_to_out(source: PathBuf, dest: PathBuf) -> AsyncOutputResult {
    let source_string = source.to_string_lossy();
    let dest_string = dest.to_string_lossy();
    info!("Starting task: copy {} to {}", source_string, dest_string);
