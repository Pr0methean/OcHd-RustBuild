...
impl<'a, UnsizedType: ?Sized + Clone, SizedType: Clone> DerefMut
    for Arcow<'a, UnsizedType, SizedType>
where
    SizedType: BorrowMut<UnsizedType>,
{
...
where
    SizedType: Borrow<UnsizedType>,
    for<'a> &'a UnsizedType: PartialEq,
{
    fn eq(&self, other: &Self) -> bool {
        if let Arcow::SharingRef(arc) = self
            && let Arcow::SharingRef(other_arc) = other
...
impl<'a, UnsizedType: ?Sized + Eq, SizedType: Clone> Eq for Arcow<'a, UnsizedType, SizedType> where
    SizedType: Borrow<UnsizedType>
{
}

impl<'a, UnsizedType: ?Sized + PartialOrd, SizedType: Clone> PartialOrd
    for Arcow<'a, UnsizedType, SizedType>
where
    SizedType: Borrow<UnsizedType>,
{
...
impl<'a, UnsizedType: ?Sized + PartialEq + Ord, SizedType: Clone> Ord
    for Arcow<'a, UnsizedType, SizedType>
where
    SizedType: Borrow<UnsizedType>,
{
...
where
    SizedType: Borrow<UnsizedType> + From<UnsizedType>,
    UnsizedType: Clone + From<SizedType>,
{
