...
use crate::materials::block::pickaxe::ore_base::{
    OreBase, DEEPSLATE, DEEPSLATE_BASE, NETHERRACK_BASE, STONE_BASE,
};
use crate::texture_base::material::{
    ColorTriad, Material, TextureSupplier, TextureUnaryFunc, TricolorMaterial, REDSTONE_ON,
};
...
pub static ALL_SUBSTRATES: Lazy<Vec<OreBase>> = Lazy::new(|| {
    vec![
        STONE_BASE.to_owned(),
        DEEPSLATE_BASE.to_owned(),
        NETHERRACK_BASE.to_owned(),
    ]
});
...
impl Ore {
    ...
    fn basic_ore_block_for_substrate(&self, substrate: ToPixmapTaskSpec) -> ToPixmapTaskSpec {
        stack!(substrate, self.default_single_layer_item())
    }

    fn raw_item_based_ore_block_for_substrate(
        &self,
        substrate: ToPixmapTaskSpec,
    ) -> ToPixmapTaskSpec {
        stack!(substrate, (self.raw_item)(self))
    }
    ...
    fn get_output_tasks(&self) -> Box<[FileOutputTaskSpec]> {
        ...
        for substrate in &self.substrates {
            output.push(out_task(
                ...
                (self.ore_block_for_substrate)(self, substrate.material.material.texture()),
            ));
        }
        if self.name != "quartz" {
            // quartz textures are defined separately in simple_pickaxe_block.rs
            output.push(out_task(
                format!("block/{}_block", self.name),
                (self.refined_block)(self),
            ));
        }
        if self.needs_refining {
            output.push(out_task(
                format!("block/raw_{}_block", self.name),
                (self.raw_block)(self),
            ));
            output.push(out_task(
                format!("item/raw_{}", self.item_name),
                (self.raw_item)(self),
            ));
            output.push(out_task(
                format!("item/{}_ingot", self.name),
                (self.refined_item)(self),
            ));
        } else {
            output.push(out_task(
...
pub static COAL: Lazy<Ore> = Lazy::new(|| {
    ...
    coal.refined_block = Box::new(|deferred_self| {
        stack_on!(
            deferred_self.colors.color,
            paint_stack!(
                deferred_self.refined_colors.highlight,
                "streaks",
                "coalBorder"
            ),
...
pub static COPPER: Lazy<Ore> = Lazy::new(|| {
    let mut copper = Ore::new("copper", c(0xff8000), c(0x915400), c(0xffa268));
...
pub static IRON: Lazy<Ore> = Lazy::new(|| {
    let mut iron = Ore::new("iron", c(0xffaf93), c(0xaf8e77), c(0xFFCDB2));
...
pub static REDSTONE: Lazy<Ore> = Lazy::new(|| {
    let mut redstone = Ore::new("redstone", ComparableColor::RED, c(0xba0000), REDSTONE_ON);
...
pub static QUARTZ: Lazy<Ore> = Lazy::new(|| {
    let mut quartz = Ore::new("quartz", c(0xe8e8de), c(0xb6a48e), ComparableColor::WHITE);
    quartz.substrates = vec![NETHERRACK_BASE.to_owned()];
    quartz.raw_item = Box::new(|_| {
        stack!(
            paint_svg_task("quartzChunk", QUARTZ.colors.color),
            paint_stack!(
                QUARTZ.colors.highlight,
                "diamond2",
                "quartzChunkTopLeftBottomRight"
            ),
